...
function updateUI() {
  ...
  if (ui.diagStatus) {
    ui.diagStatus.textContent = diagStatus;
    const status = diagStatus.startsWith("fail")
      ? "fail"
      : diagStatus === "ok"
      ? "ok"
      : diagStatus === "running"
      ? "running"
      : "idle";
    ui.diagStatus.dataset.state = status;
    if (ui.diagPill) ui.diagPill.dataset.state = status;
  }
  const c = ui.controls;
...
function createNumberControl(labelText, key, options, onChange) {
  const label = el("label", "control-field");
  if (options?.width) label.classList.add(options.width);
  const span = el("span", "control-label", labelText);
...
function createCheckboxControl(labelText, key, onChange) {
  const label = el("label", "checkbox-field");
  const input = document.createElement("input");
  input.type = "checkbox";
  input.className = "checkbox-toggle";
  input.checked = !!state[key];
  input.addEventListener("change", (e) => {
    onChange(e.target.checked);
    updateUI();
  });
  const span = el("span", "checkbox-label", labelText);
...
function initUI() {
  ...
  root.className = "page";
  const container = el("div", "app-shell");
  root.appendChild(container);

  const hero = el("header", "hero");
  hero.appendChild(el("span", "hero-chip", "Portfolio build"));
  hero.appendChild(el("h1", "hero-title", "Image-GS — Browser Gaussian Splatting"));
  hero.appendChild(
    el(
      "p",
      "hero-subtitle",
      "Train and render photorealistic 2D Gaussian splats directly in the browser. Drag in an image, tune the optimization budget, and watch a worker pool converge in real time."
    )
  );
  const heroBadges = el("div", "hero-badges");
  heroBadges.append(
    el("span", "hero-badge", "Pure JavaScript worker pool"),
    el("span", "hero-badge", "Max 200k splats"),
    el("span", "hero-badge", "No WebGL or WASM required")
  );
  hero.appendChild(heroBadges);
  container.appendChild(hero);

  const main = el("div", "content-stack");
  container.appendChild(main);

  const controlsCard = el("section", "panel controls-panel");
  controlsCard.appendChild(el("h2", "panel-title", "Upload & iterate"));
  controlsCard.appendChild(
    el(
      "p",
      "panel-subtitle",
      "Load an image, define the reconstruction budget, and iterate on settings with responsive feedback."
    )
  );

  const controlsRow = el("div", "control-bar");
  ...
  controlsRow.appendChild(buttonRow);

  const trainButton = el("button", "btn btn-primary", "Train");
  trainButton.addEventListener("click", train);
  buttonRow.appendChild(trainButton);
  ui.trainButton = trainButton;

  const renderButton = el("button", "btn btn-secondary", "Render once");
  renderButton.addEventListener("click", handleRenderOnce);
  buttonRow.appendChild(renderButton);
  ui.renderButton = renderButton;

  const stopButton = el("button", "btn btn-ghost", "Stop");
  stopButton.addEventListener("click", handleStop);
  buttonRow.appendChild(stopButton);
  ui.stopButton = stopButton;

  const saveButton = el("button", "btn btn-ghost", "Save .igs");
  saveButton.addEventListener("click", handleSaveModel);
  buttonRow.appendChild(saveButton);
  ui.saveButton = saveButton;

  const diagContainer = el("div", "diag-status");
  const diagButton = el("button", "btn btn-ghost btn-small", "Diagnostics");
  diagButton.addEventListener("click", runDiagnostics);
  const diagSpan = el("span", "diag-status-text", diagStatus);
  diagContainer.append(diagButton, diagSpan);
  controlsRow.appendChild(diagContainer);
  ui.diagStatus = diagSpan;
  ui.diagPill = diagContainer;

  main.appendChild(controlsCard);

  const workspaceCard = el("section", "panel workspace-panel");
  workspaceCard.appendChild(el("h2", "panel-title", "Training workspace"));
  workspaceCard.appendChild(
    el(
      "p",
      "panel-subtitle",
      "Fine-tune hyperparameters, inspect reconstruction metrics, and understand how the splats evolve."
    )
  );

  const grid = el("div", "workspace-grid");
  workspaceCard.appendChild(grid);
  main.appendChild(workspaceCard);

  const leftCol = el("div", "left-stack");
  grid.appendChild(leftCol);

  const group1 = el("div", "control-group");
  group1.appendChild(
    createNumberControl("K", "K", { min: 1, max: 64 }, (value, input) => {
  ...
  group1.appendChild(
    createNumberControl("Budget (N)", "budget", { min: 128, max: 200000 }, (value, input) => {
      const parsed = parseInt(value, 10);
      state.budget = Number.isFinite(parsed) ? clamp(parsed, 128, 200000) : 128;
      input.value = String(state.budget);
    })
  );
  group1.appendChild(
    createNumberControl("λ_init", "lambdaInit", { min: 0, max: 1, step: 0.05 }, (value, input) => {
  ...
  group1.appendChild(
    createNumberControl("Steps", "steps", { min: 100, max: 20000 }, (value, input) => {
      const parsed = parseInt(value, 10);
  ...
  group1.appendChild(
    createNumberControl("Delay (ms)", "stepDelayMs", { min: 0, max: 1000 }, (value, input) => {
      const parsed = parseInt(value, 10);
  ...
  const group2 = el("div", "control-group");
  group2.appendChild(
    createNumberControl("Pool size", "poolSize", { min: 1, max: 16 }, (value, input) => {
      const parsed = parseInt(value, 10);
  ...
  group2.appendChild(
    createNumberControl("Tile W", "tileW", { min: 8, max: 128 }, (value, input) => {
      const parsed = parseInt(value, 10);
  ...
  group2.appendChild(
    createNumberControl("Tile H", "tileH", { min: 8, max: 128 }, (value, input) => {
      const parsed = parseInt(value, 10);
  ...
  group2.appendChild(
    createNumberControl("Rebin every", "rebinEvery", { min: 0, max: 500 }, (value, input) => {
      const parsed = parseInt(value, 10);
  ...
  const group3 = el("div", "control-group");
  group3.appendChild(
    createNumberControl("LR Color", "lrColor", { min: 0.05, max: 1, step: 0.05 }, (value, input) => {
      const parsed = parseFloat(value);
  ...
  group3.appendChild(
    createNumberControl("LR μ", "lrMu", { min: 0.05, max: 1, step: 0.05 }, (value, input) => {
      const parsed = parseFloat(value);
  ...
  group3.appendChild(
    createNumberControl("LR shape", "lrShape", { min: 0.05, max: 1, step: 0.05 }, (value, input) => {
      const parsed = parseFloat(value);
  ...
  const metricsInfo = el("div", "stats-card", "Image: 0×0 • Gaussians: 0 • Step: 0 • Pool: 1");
  statsGrid.appendChild(metricsInfo);
  ui.metricsInfo = metricsInfo;

  const psnrInfo = el("div", "stats-card", "PSNR: — dB");
  statsGrid.appendChild(psnrInfo);
  ui.psnrInfo = psnrInfo;
  leftCol.appendChild(statsGrid);

  const rightCol = el("div", "narrative prose");
  grid.appendChild(rightCol);
  rightCol.appendChild(el("h2", null, "Engineered for performance"));
  rightCol.appendChild(
    el(
      "p",
      null,
      "A dedicated worker pool keeps the renderer responsive. Each worker advances EM updates on its stripe, returning partial accumulators for the main thread to reduce without blocking the UI."
    )
  );
  rightCol.appendChild(el("h2", null, "Adaptive tiling & binning"));
  rightCol.appendChild(
    el(
      "p",
      null,
      "Spatial bins rebuild automatically as μ, θ, and σ evolve, so every pixel query stays local. Tweak tile sizes on the fly to balance cache friendliness and parallelism."
    )
  );
  rightCol.appendChild(el("h2", null, "Thoughtful UX details"));
  rightCol.appendChild(
    el(
      "p",
      null,
      "Real-time metrics, PSNR tracking, and one-click diagnostics make this demo presentation ready. Everything runs in pure JavaScript with a CSP-safe build pipeline."
    )
  );
}
